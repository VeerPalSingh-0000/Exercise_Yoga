  ...
  useEffect(() => {
    ...
    if (savedGoals) {
      try {
        ...
        console.error("Failed to parse nutrition goals from localStorage:", error);
      }
    }
  }, []);
        ...
        <div className="max-w-7xl mx-auto px-2 sm:px-4 lg:px-8">
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-3 mb-6">
            ...
            <div className="bg-gray-900 p-4 rounded-xl shadow-lg border border-gray-700 flex flex-col min-h-[220px]">
              <h3 className="text-base font-semibold mb-3 text-gray-100">Workout Trend (7 Days)</h3>
            ...
            <div className="lg:col-span-2 grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="bg-gray-900 p-4 rounded-xl shadow-lg border border-gray-700">
                <ProgressBar label="Workout Duration" percentage={goals.WORKOUT_DURATION > 0 ? (progressData.duration / goals.WORKOUT_DURATION) * 100 : 0} valueText={`${progressData.duration} / ${goals.WORKOUT_DURATION} mins`} icon={<FaPersonRunning size="1.1em" />} isLoading={isLoadingProgress} goal={goals.WORKOUT_DURATION} onGoalChange={v => updateGoal("WORKOUT_DURATION", v)} />
              </div>
              <div className="bg-gray-900 p-4 rounded-xl shadow-lg border border-gray-700">
                <ProgressBar label="Daily Steps" percentage={goals.DAILY_STEPS > 0 ? (progressData.steps / goals.DAILY_STEPS) * 100 : 0} valueText={`${progressData.steps} / ${goals.DAILY_STEPS} steps`} icon={<FaWalking size="1.1em" />} isLoading={isLoadingProgress} goal={goals.DAILY_STEPS} onGoalChange={v => updateGoal("DAILY_STEPS", v)} />
              </div>
            </div>
          </div>
          ...
          <div className="bg-gray-900 p-4 rounded-xl shadow-lg mb-6 border border-gray-700">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-base font-semibold text-gray-100 flex items-center"><FaSeedling className="mr-2 text-green-400" />Today's Nutrition</h3>
              <button onClick={() => setShowNutritionEdit(true)} className="flex items-center bg-yellow-500 px-3 py-1 rounded-md hover:bg-yellow-400 text-xs font-medium text-gray-900" title="Edit nutrition goals"><FiEdit className="mr-1" /> Edit</button>
            </div>
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
              {MACRO_NUTRIENTS.map((n) => (
                <CircularProgress
                  ...
                  percentage={((nutritionData[n.toLowerCase()] || 0) / (nutritionGoals[n] || 1)) * 100}
                  current={nutritionData[n.toLowerCase()] || 0}
                  goal={nutritionGoals[n]}
                  unit={NUTRITION_UNITS[n]}
                  color={n === "CALORIES" ? "#f59e0b" : n === "PROTEIN" ? "#ef4444" : n === "CARBS" ? "#3b82f6" : "#10b981"}
                  isLoading={isLoadingNutrition}
                />
              ))}
            </div>
            <button onClick={() => navigate('/nutrition-history')} className="bg-green-600 px-4 py-2 rounded-md hover:bg-green-500 text-sm mt-4">View Nutrition History</button>
            {showNutritionEdit && (
              <NutritionGoalsModal
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-5 mb-6">
            <div className="lg:col-span-3 bg-gray-900 p-4 rounded-xl shadow-lg border border-gray-700">
              <div className="flex justify-between items-center mb-4 flex-wrap gap-2">
                <h3 className="text-base font-semibold text-gray-100 flex items-center"><FaTable className="mr-2 text-emerald-400" /> Weekly Schedule</h3>
                <div className="flex items-center space-x-2">
                  {isWeeklyPlanEditable ? (
                    <>
                      <button onClick={handleWeeklyPlanSave} className="flex items-center bg-emerald-600 px-3 py-1 rounded-md hover:bg-emerald-500 text-xs font-medium"><FiSave className="mr-1" /> Save</button>
                      {/* CORRECTED: Use handler that resets state on cancel. */}
                      <button onClick={handleWeeklyPlanCancel} className="flex items-center bg-gray-600 px-3 py-1 rounded-md hover:bg-gray-500 text-xs font-medium"><FiX className="mr-1" /> Cancel</button>
                    </>
                  ) : (
                    <button onClick={() => setIsWeeklyPlanEditable(true)} className="flex items-center bg-yellow-500 px-3 py-1 rounded-md hover:bg-yellow-400 text-xs font-medium text-gray-900"><FiEdit className="mr-1" /> Change</button>
                  )}
                </div>
              </div>
              <WeeklyPlan schedule={isWeeklyPlanEditable ? tempSchedule : schedule} isEditable={isWeeklyPlanEditable} onExerciseClick={handleExerciseNavigation} onExerciseChange={handleWeeklyExerciseChange} highlightIndex={todayIndex} />
            </div>
            // Replace the entire "Daily Routine" div with this corrected code
<div className="lg:col-span-2 bg-gray-900 p-4 rounded-xl shadow-lg border border-gray-700 flex flex-col">
  <div className="flex justify-between items-center mb-4 flex-wrap gap-2">
    ...
    <div className="flex items-center space-x-2">
      {isDailyPlanEditable ? (
        <>
          <button onClick={handleDailyPlanSave} className="flex items-center bg-emerald-600 px-3 py-1 rounded-md hover:bg-emerald-500 text-xs font-medium"><FiSave className="mr-1" /> Save</button>
          <button onClick={handleDailyPlanCancel} className="flex items-center bg-gray-600 px-3 py-1 rounded-md hover:bg-gray-500 text-xs font-medium"><FiX className="mr-1" /> Cancel</button>
        </>
      ) : (
        <button onClick={() => setIsDailyPlanEditable(true)} className="flex items-center bg-yellow-500 px-3 py-1 rounded-md hover:bg-yellow-400 text-xs font-medium text-gray-900"><FiEdit className="mr-1" /> Change</button>
      )}
    </div>
  </div>
  ...
  <div className="flex flex-col gap-2 overflow-y-auto px-2" style={{ flex: 1 }}>
    {isDailyPlanEditable ? (
      <>
        ...
        {tempFixedExercises.map((exercise, idx) => (
          ...
          <div key={idx} className="flex items-center gap-2">
            ...
            <button
              onClick={() => {
                const newArr = tempFixedExercises.filter((_, i) => i !== idx);
                setTempFixedExercises(newArr);
              }}
        ...
        <button
          onClick={() => setTempFixedExercises([...tempFixedExercises, ""])}
      ...
      dailyExercisesToShow.map((ex, exIndex) => (
        <span key={exIndex} onClick={() => handleExerciseNavigation(ex)} className="bg-gray-700 capitalize text-gray-200 px-3 py-1.5 rounded-full text-xs font-medium cursor-pointer hover:bg-emerald-600 transition-colors">
          {ex}
        </span>
      ))
    ) : (
      <p className="text-xs text-gray-500">
        No daily exercises set.
          ...
          <div className="bg-gray-900 p-4 rounded-xl shadow-lg text-center border border-gray-700">
            <h3 className="text-base font-semibold mb-3 text-gray-100 flex items-center justify-center"><GiLotus className="mr-2 text-purple-400" /> Yoga & Flexibility</h3>
            <div className="max-w-xs mx-auto mb-4">
              <ProgressBar label="Flexibility Goal" percentage={goals.YOGA_DURATION > 0 ? (progressData.yogaDuration / goals.YOGA_DURATION) * 100 : 0} valueText={`${progressData.yogaDuration} / ${goals.YOGA_DURATION} mins`}
                // CORRECTED: Changed icon for UI consistency.
                icon={<GiLotus size="1.1em" className="text-purple-400" />}
                isLoading={isLoadingProgress} goal={goals.YOGA_DURATION} onGoalChange={(v) => updateGoal("YOGA_DURATION", v)}
              />
            </div>
            <button onClick={() => navigate("/yoga")} className="inline-flex items-center bg-purple-600 px-4 py-2 rounded-md font-semibold text-white hover:bg-purple-500 text-sm">
