...
const FOOD_CATEGORIES = {
  "Indian Meals": [
    ...
    { name: "Plain Paratha (1 medium)", calories: 175, protein: 4, carbs: 28, fats: 6, icon: "🍞" },
    ...
    { name: "Upma (1 plate, ~200g)", calories: 220, protein: 5, carbs: 38, fats: 6, icon: "🥣" },
  ],
  "Indian Sabjis": [
    ...
    { name: "Phali ki Sabji (1 bowl)", calories: 53, protein: 3, carbs: 8, fats: 2, icon: "🌿" },
    { name: "Bhindi Masala (1 bowl)", calories: 110, protein: 2, carbs: 14, fats: 6, icon: "🧅" },
    { name: "Baingan Bharta (1 bowl)", calories: 90, protein: 2, carbs: 13, fats: 4, icon: "🍆" },
    { name: "Mix Veg (1 bowl)", calories: 130, protein: 4, carbs: 15, fats: 6, icon: "🥗" },
    { name: "Matar Paneer (1 bowl)", calories: 228, protein: 10, carbs: 14, fats: 14, icon: "🧀" },
  ...
  Vegetables: [
    ...
    { name: "Broccoli (1 cup)", calories: 55, protein: 3.7, carbs: 11, fats: 0.6, icon: "🥦" },
    { name: "Spinach (1 cup)", calories: 7, protein: 0.9, carbs: 1.1, fats: 0.1, icon: "🌿" },
    { name: "Tomato (1 medium)", calories: 22, protein: 1.1, carbs: 4.8, fats: 0.2, icon: "🍅" },
    { name: "Cucumber (1 medium)", calories: 16, protein: 0.7, carbs: 3.8, fats: 0.1, icon: "🥒" },
    { name: "Bell Pepper (1 medium)", calories: 24, protein: 1, carbs: 6, fats: 0.2, icon: "🌶" },
    { name: "Cauliflower (1 cup)", calories: 25, protein: 2, carbs: 5, fats: 0.1, icon: "🥬" },
    { name: "Zucchini (1 medium)", calories: 33, protein: 2.4, carbs: 6.1, fats: 0.4, icon: "🥒" },
    { name: "Green Beans (1 cup)", calories: 31, protein: 2, carbs: 7, fats: 0.2, icon: "🫘" },
    { name: "Mushrooms (1 cup)", calories: 15, protein: 2.2, carbs: 2.3, fats: 0.2, icon: "🍄" },
  ],
  Fat: [
    { name: "Olive Oil (1 tbsp)", calories: 119, protein: 0, carbs: 0, fats: 14, icon: "🛢" },
    { name: "Butter (1 tbsp)", calories: 102, protein: 0.1, carbs: 0, fats: 12, icon: "🧈" },
    { name: "Almonds (1 oz)", calories: 164, protein: 6, carbs: 6, fats: 14, icon: "🌰" },
    { name: "Walnuts (1 oz)", calories: 185, protein: 4.3, carbs: 4, fats: 18.5, icon: "🌰" },
    { name: "Chia Seeds (1 oz)", calories: 137, protein: 4.4, carbs: 12, fats: 8.6, icon: "🌱" },
    { name: "Flaxseeds (1 tbsp)", calories: 55, protein: 1.9, carbs: 3, fats: 4.3, icon: "🌾" },
    { name: "Peanut Butter (2 tbsp)", calories: 190, protein: 7, carbs: 8, fats: 16, icon: "🥜" },
...
const useRepTracking = (currentUser, logDate) => {
  ...
  const fetchRepLogs = useCallback(async () => {
    ...
    try {
      ...
      console.error("Error fetching rep logs:", error);
    } finally {
      setIsLoading(false);
    }
  }, [currentUser, logDate]);
...
const Toast = memo(({ toast, onRemove }) => {
  ...
  return (
    <motion.div
      ...
      className={`fixed bottom-5 right-5 p-4 rounded-xl shadow-2xl text-white border backdrop-blur-md bg-gradient-to-br ${
        styles[toast.type]
      } bg-opacity-80 z-50`}
    >
...
const GeneralActivityForm = memo(({ onSubmit, isSubmitting }) => {
  ...
  const handleExerciseToggle = (exName) =>
    setExercises((prev) =>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          ...
          <div>
            <label className="text-sm font-semibold text-gray-300 mb-2 flex items-center">
              <FiClock className="mr-2 text-emerald-400" /> Workout Duration
            </label>
          ...
          <div>
            <label className="text-sm font-semibold text-gray-300 mb-2 flex items-center">
              <FaWalking className="mr-2 text-sky-400" /> Step Count
            </label>
            <div className="relative">
              <input
                type="number"
        ...
        <div className="space-y-4">
          <label className="text-sm font-semibold text-gray-300 flex items-center">
            <FiTarget className="mr-2 text-purple-400" /> Target Muscle Groups{" "}
        ...
        >
          {isSubmitting ? (
            <>
              <FiLoader className="mr-3 animate-spin" /> Logging...
            </>
          ) : (
            <>
              <FiCheckCircle className="mr-3" /> Log Workout
            </>
          )}
        </motion.button>
      </form>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          <label className="text-sm font-semibold text-gray-300 flex items-center mb-2">
            <FaDumbbell className="mr-2 text-orange-400" /> Exercise Name
          </label>
          <input
            type="text"
            value={exerciseName}
        ...
        <div className="space-y-4">
          <h4 className="text-sm font-semibold text-gray-300 flex items-center">
            <HiSparkles className="mr-2 text-yellow-400" /> Sets & Reps
          </h4>
          <AnimatePresence>
            {sets.map((set, index) => (
              <motion.div
...
const NutritionForm = memo(({ onSubmit, isSubmitting }) => {
  ...
  const handleSubmit = (e) => {
    ...
    const food = FOOD_CATEGORIES[selectedCategory].find(
  ...
  const foodsInCategory = selectedCategory
    ? FOOD_CATEGORIES[selectedCategory]
    : [];
  ...
  return (
    <FormCard>
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          <label className="text-sm font-semibold text-gray-300 mb-2 flex items-center">
            <FaSeedling className="mr-2 text-green-400" /> Food Category
          </label>
          <select
    ...
    .map(f => (
      ...
      >
        ...
        {selectedFood && (
          <div>
            <label className="text-sm font-semibold text-gray-300 mb-2 flex items-center">
              <FiTarget className="mr-2 text-sky-400" /> Quantity/Servings
            </label>
            <input
              type="number"
              value={quantity}
          ...
          {selectedFoodData && (
            ...
            >
              <h5 className="text-sm font-semibold text-gray-300 mb-3 flex items-center">
                <FaFire className="mr-2 text-orange-400" /> Nutrition Preview (x
                {quantity || 0})
              </h5>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-center">
                <div>
                  <p className="text-xl font-bold text-orange-400">
                    {Math.round(selectedFoodData.calories * quantity || 0)}
                  </p>
                  <p className="text-xs text-gray-400">Calories</p>
                </div>
                <div>
                  <p className="text-xl font-bold text-sky-400">
                    {Math.round(selectedFoodData.protein * quantity * 10) /
                      10 || 0}
                    g
                  </p>
                ...
                <div>
                  <p className="text-xl font-bold text-green-400">
                    {Math.round(selectedFoodData.carbs * quantity * 10) / 10 ||
                      0}
                    g
                  </p>
                ...
                <div>
                  <p className="text-xl font-bold text-yellow-400">
                    {Math.round(selectedFoodData.fats * quantity * 10) / 10 ||
                      0}
                    g
                  </p>
        ...
        {selectedFood && (
          ...
          >
            {isSubmitting ? (
              <>
                <FiLoader className="mr-3 animate-spin" /> Adding...
              </>
            ) : (
              <>
                <FiCheckCircle className="mr-3" /> Add to Log
              </>
            )}
          </motion.button>
        )}
...
const Exercise = () => {
  ...
  const {
    ...
    refetch: refetchReps,
  } = useRepTracking(currentUser, logDate);
  ...
  const createLogHandler = (collectionName, successMessage, errorMessage) =>
    useCallback(
      async (data) => {
  ...
  const handleLogGeneralActivity = createLogHandler(
    "workoutLogs",
    "Workout logged! 💪",
    "Log failed"
  );
  const handleLogReps = createLogHandler(
    "repLogs",
    "Exercise logged! 🏋️‍♂️",
    "Log failed"
  );
  const handleLogNutrition = createLogHandler(
    "nutritionLogs",
    "Nutrition logged! 🥗",
    "Log failed"
  );

  const handleDeleteRepLog = useCallback(
    async (logId) => {
      try {
            ...
            {[
              {
                ...
                icon: <FiActivity className="w-6 h-6 text-emerald-300" />,
                gradient: "from-emerald-600/70 to-green-700/70",
                Form: GeneralActivityForm,
                handler: handleLogGeneralActivity,
              },
              {
                id: "lifts",
                title: "Strength Training",
                icon: <FaDumbbell className="w-6 h-6 text-orange-300" />,
                gradient: "from-orange-600/70 to-amber-700/70",
                Form: RepTrackingForm,
                handler: handleLogReps,
              },
              {
                id: "nutrition",
                title: "Nutrition Tracking",
                icon: <FaSeedling className="w-6 h-6 text-green-300" />,
                gradient: "from-green-600/70 to-emerald-700/70",
                Form: NutritionForm,
                handler: handleLogNutrition,
              },
                        ...
                        <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                          <div>
                            <h4 className="text-xl font-bold mb-4 flex items-center">
                              <FiPlus className="mr-3 text-orange-400" /> Log
                              New Exercise
                            </h4>
                            <Form
                              onSubmit={handler}
                          ...
                          <div>
                            <h4 className="text-xl font-bold mb-4 flex items-center">
                              <FiTrendingUp className="mr-3 text-emerald-400" />{" "}
                              Today's Training Log
                            </h4>
                            {isLoadingReps ? (
                              <div className="text-center p-8 bg-slate-900/50 rounded-2xl">
                                  ...
                                  {repLogs.map((log) => (
                                    ...
                                    >
                                      <div className="flex justify-between items-start mb-3">
                                        <h5 className="font-bold text-lg text-orange-400">
                                          {log.exerciseName}
                                        </h5>
                                        <button
                                          onClick={() =>
                                            handleDeleteRepLog(log.id)
                                          }
                                      ...
                                      <div className="space-y-2">
                                        {log.sets.map((set, i) => (
                                          <div
                                            key={i}
                                            className="flex justify-between bg-slate-800/60 rounded-lg p-2 text-sm"
                                          >
