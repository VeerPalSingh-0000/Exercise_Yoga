            ...
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-6">
                {bodyFatData.map(bf => (
                    <motion.button
                        key={bf.range}
                        onClick={() => onSelect(bf)}
                        className="bg-gray-800 border border-gray-700 rounded-xl p-4 text-center transition-all hover:border-orange-500 hover:scale-105 overflow-hidden"
                        whileHover={{ y: -5 }}
                    >
                        <img src={bf.image} alt={`Body fat ${bf.range}`} className="rounded-lg mb-4 mx-auto w-full h-auto aspect-[2/4] object-cover" />
                        <h3 className="text-xl font-bold">{bf.range}</h3>
